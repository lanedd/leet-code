...
class Solution:
    def trapRainWater(self, heightMap: List[List[int]]) -> int:
        # Address empty heightMap
        if not heightMap or not heightMap[0]:
            return 0
        # Create map of Nodes
        node_map = NodeMap(height=len(heightMap), width=len(heightMap[0]))
        node_map.create_map(heightMap)
        node_map.map_neighbors()
        # Start heap as the edges of the map
        heap = [node_map.map[h_i][w_i] for h_i in range(1, node_map.height) for w_i in (0, node_map.width - 1)]
        heap += node_map.map[0] + node_map.map[node_map.height - 1]
        for node in heap:
            node.visit()
        heapq.heapify(heap)
        # Find water level at each tile
        max_height = 0
        water = 0
        while heap:
            explore_node = heapq.heappop(heap)
            neighbors, height = explore_node.explore()
...
class Node:
    ...
    # Allows for sorting in lists and heaps
    def __lt__(self, other):
        return self.height < other.height

    def explore(self):
        self.explored = True
        return self.neighbors, self.height
...
class NodeMap:
    ...
    def create_map(self, array):
        for h_i in range(self.height):
            row = [Node(loc=(h_i, w_i), height=array[h_i][w_i]) for w_i in range(self.width)]
            self.map.append(row)

    def map_neighbors(self):
        for h_i in range(self.height):
            for w_i in range(self.width):
                node = self.map[h_i][w_i]
                self.node_neighbors(node)

    def node_neighbors(self, node):
        # Do vertical neighbors
        h_i_max = self.height - 1
        h_i_min = 0
        h_i = node.loc[0] + 1
        w_i = node.loc[1]
        if h_i <= h_i_max:
            node.neighbors.append(self.map[h_i][w_i])
        h_i = node.loc[0] - 1
        if h_i >= h_i_min:
            node.neighbors.append(self.map[h_i][w_i])
        # Do horizontal neighbors
        w_i_max = self.width - 1
        w_i_min = 0
        h_i = node.loc[0]
        w_i = node.loc[1] + 1
        if w_i <= w_i_max:
...
# class Solution:
#     def trapRainWater(self, heightMap: List[List[int]]) -> int:
#         # Check for heightMap with no nodes.
#         if len(heightMap) == 0:
#             return 0
#         elif len(heightMap[0]) == 0:
#             return 0
#         height = len(heightMap)
#         width = len(heightMap[0])
#         node_map = NodeMap(height, width)
#
#         # Find maximum wall level
#         max_wall = 0
#         for row in heightMap:
#             max_wall = max(max_wall, max(row))
#
#         water_count = 0
#         for level in range(1, max_wall+1):
#             node_map.set_map(heightMap, level)
#             node_map.explore_nodes()
#             water = node_map.count_water()
#             water_count += water
#         return water_count
#
#
...
#         for h_i in range(self.height):
#             self.map[h_i] = [Node((h_i, w_i)) for w_i in range(self.width)]
#         # populate neighbors
#         for h_index in range(1, self.height - 1):
#             for w_index in range(1, self.width - 1):
#                 this_node = self.map[h_index][w_index]
#                 for offset in (-1, 1):
#                     this_node.neighbors.append(self.map[h_index + offset][w_index])
#                     this_node.neighbors.append(self.map[h_index][w_index + offset])
#         self.node_list = list()
#         for h_i in range(self.height):
#             for w_i in range(self.width):
#                 self.node_list.append(self.map[h_i][w_i])
#
#     def set_map(self, height_map, level):
#         # Set inner map
#         for h_i in range(1, self.height - 1):
#             for w_i in range(1, self.width - 1):
#                 this_node = self.map[h_i][w_i]
#                 # this_node.visited = False
#                 if level <= height_map[h_i][w_i]:
#                     this_node.visited = True
#                     this_node.wall = True
#                     this_node.water = False
#                 else:
#                     this_node.visited = False
#                     this_node.wall = False
#                     this_node.water = None
#         # Set edges
#         for h_i in range(self.height):
#             for w_i in (0, self.width - 1):
#                 self._set_edge_nodes(height_map, level, h_i, w_i)
#         for h_i in (0, self.height - 1):
#             for w_i in range(self.width):
#                 self._set_edge_nodes(height_map, level, h_i, w_i)
#
#     def _set_edge_nodes(self, height_map, level, h_i, w_i):
#         this_node = self.map[h_i][w_i]
#         this_node.visited = True
#         if level <= height_map[h_i][w_i]:
#             this_node.wall = True
...
#                 continue
#             holds_water = True
#             explore_list = [root_node, ]
#             explored_set = set()
#             while len(explore_list) > 0:
#                 node = explore_list.pop()
#                 water, neighbors = node.explore()
#                 if not water:
#                     holds_water = False
#                 if node.visited:
#                     continue
#                 explore_list.extend(neighbors)
#                 node.visited = True
#                 explored_set.add(node)
#             for node in explored_set:
#                 node.water = holds_water
#
...
#
# class Node:
#     def __init__(self, loc, visited=False, water=None, wall=None, neighbors=None):
#         self.loc = loc
...
#         if self.wall:
#             return True, list()
#         if self.water is not None and not self.water:
#             return False, self.neighbors
#         else:
#             return True, self.neighbors
...
for my_input, ans in (
    ...
    my_sol = Solution().trapRainWater(my_input)
    print(my_sol, ans)
